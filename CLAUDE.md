...
\- CORS middleware configured with `allow\_origins=\["\*"]` (open — fine for local dev)
...
\- API base URL (`http://localhost:8000`) is hardcoded in `App.jsx` — use a `VITE\_API\_URL` env var when this needs to change.
...
\- CORS: currently `allow\_origins=\["\*"]` — tighten to `\["http://localhost:5173"]` when ready
